 ...
 *
 * 🔴 Именно статика Nuxt, а НЕ `/api/files/legal-documents/` — тот роут на
 * проде отдаёт 404: в рантайм-образ главного сервера папка `public/legal-documents`
 * не копируется, а обработчик читает её из рабочего каталога. Локально это
 * корень проекта, поэтому дефект виден только на проде.
 */
const LEGAL_DOCUMENTS_PATH = "/legal-documents";
